...
@Suite struct BrowserURLTests {
    @Test func httpURL() throws {
        let url = try BrowserURL("http://example.org/index.html")
    ...
    @Test func httpsURL() throws {
        let url = try BrowserURL("https://example.org/")
    ...
    @Test func customPort() throws {
        let url = try BrowserURL("http://localhost:8080/path")
    ...
    @Test func defaultPath() throws {
        let url = try BrowserURL("http://example.org")
        #expect(url.path == "/")
    }

    @Test func unsupportedSchemeThrows() {
        #expect(throws: BrowserError.self) {
            try BrowserURL("ftp://example.org/")
        }
    }

    @Test func missingSeparatorThrows() {
        #expect(throws: BrowserError.self) {
            try BrowserURL("http:example.org/")
        }
    }
}
