...
struct LayoutTests {
    @Test func `single word near origin`() throws {
        let list = Layout(parse("hello")).displayList
        #expect(list.count == 1)
    ...
    @Test func `one item per word`() {
        #expect(Layout(parse("hello there world")).displayList.count == 3)
    }

    @Test func `long text wraps to a new line`() throws {
        let list = Layout(parse(String(repeating: "word ", count: 100))).displayList
    ...
    @Test func `mixed sizes share a baseline`() throws {
        ...
        let list = Layout(parse("<big>Big</big> small")).displayList
    ...
    @Test func `empty tokens produce no items`() {
        #expect(Layout(parse("")).displayList.isEmpty)
    }
}
