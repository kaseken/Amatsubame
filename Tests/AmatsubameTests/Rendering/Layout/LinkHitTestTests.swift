...
struct LinkHitTestTests {
    ...
    @Test func `resolves a relative href against the base url`() throws {
        ...
        let links = [LinkTarget(rect: box, href: "next.html", nodePath: nil)]
        let url = try #require(hitTestLink(at: Point(x: 10, y: 10), links: links, relativeTo: base))
        #expect(url.absoluteString == "https://example.com/dir/next.html")
    }

    @Test func `resolves an absolute href`() throws {
        let links = [LinkTarget(rect: box, href: "https://other.example/a", nodePath: nil)]
        let url = try #require(hitTestLink(at: Point(x: 5, y: 5), links: links, relativeTo: URL(string: "https://example.com/")))
        #expect(url.absoluteString == "https://other.example/a")
    }

    @Test func `misses when the point is outside every target`() {
        let links = [LinkTarget(rect: box, href: "next.html", nodePath: nil)]
        #expect(hitTestLink(at: Point(x: 200, y: 10), links: links, relativeTo: URL(string: "https://example.com/")) == nil)
    }

    @Test func `picks the topmost overlapping target`() throws {
        let base = URL(string: "https://example.com/")
        let links = [
            LinkTarget(rect: box, href: "under.html", nodePath: nil),
            LinkTarget(rect: box, href: "over.html", nodePath: nil),
        ]
        let url = try #require(hitTestLink(at: Point(x: 10, y: 10), links: links, relativeTo: base))
